...
void_region = outer_boundary_cell & ~detector_cell.region & ~first_wall_region & ~second_wall_region
void_cell = openmc.Cell(name="air_cell", fill=AIR_MATERIAL, region=void_region)

universe = openmc.Universe(cells=[detector_cell, void_cell, first_wall_cell, second_wall_cell])
geometry = openmc.Geometry(universe)

geometry.export_to_xml()
materials.export_to_xml()

plot_geometry(materials=materials, width=110, height=110, saving_figure=True)
...
# load the drm into a npz file
np.savez("drm_spectrum_wall.npz", energy=energy, energy_bins=energy_bins, spectrums=spectrums, spectrum_std=spectrum_std)